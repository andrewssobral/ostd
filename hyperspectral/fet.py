...
for file_gt, file_fg in zip(files_gt, files_fg):
    ...
    img_fg_raw = cv2.imread(path_fg + file_fg,cv2.IMREAD_GRAYSCALE)
    img_fg = img_fg_raw;
    #img_fg = cv2.resize(img_fg_raw, (0,0), fx=0.5, fy=0.5)
    #img_fg = cv2.resize(img_fg_raw, (120, 120)) 
    img_gt_raw = cv2.imread(path_gt + file_gt,cv2.IMREAD_GRAYSCALE)
    #img_gt = cv2.resize(img_gt_raw, (0,0), fx=0.5, fy=0.5)
    newx,newy = img_fg.shape[1],img_fg.shape[0] #new size (w,h)
    img_gt = cv2.resize(img_gt_raw,(newx,newy))
